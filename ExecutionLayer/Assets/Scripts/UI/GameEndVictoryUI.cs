...
/// <summary>
/// Runtime victory overlay for the shop assistant game-end screen.
/// </summary>
public sealed class GameEndVictoryUI : MonoBehaviour
{
    ...
    [SerializeField] private Key debugShowKey = Key.N;
    ...
    [SerializeField] private float titleWidth = 1390f;
    [SerializeField] private float titleHeight = 928f;
    [SerializeField] private float titleAnchoredX = -20f;
    [SerializeField] private float titleAnchoredY = 64f;
    ...
    [SerializeField] private float stampIntroSeconds = 0.74f;
    ...
    private static GameEndVictoryUI instance;
    ...
    private VictoryStatRowData[] statRowsData;
    private Coroutine showRoutine;
    private string gameEndInfoJson;
    private static int lastDebugInputFrame = -1;
    private const string TestGameEndInfoJson = "{\"result\":\"victory\",\"isVictory\":true,\"stats\":{\"roundCount\":18,\"bestSellingItem\":{\"itemId\":\"item:meat\",\"shortItemId\":\"meat\",\"name\":\"烤肉\",\"quantity\":87},\"mostPurchasedItem\":{\"itemId\":\"item:gold\",\"shortItemId\":\"gold\",\"name\":\"黄金\",\"quantity\":356},\"totalSoldQuantity\":512,\"totalIncome\":12800}}";
    ...
    private static void Bootstrap()
    {
        ...
        var root = new GameObject("UI_GameEndVictory");
        root.AddComponent<GameEndVictoryUI>();
    }
    ...
    public static void ShowVictory(string gameEndInfoJson)
    {
        var ui = instance != null ? instance : FindObjectOfType<GameEndVictoryUI>();
        if (ui == null)
        {
            var root = new GameObject("UI_GameEndVictory");
            ui = root.AddComponent<GameEndVictoryUI>();
        }
    ...
    public static void HandleDebugInput()
    {
        ...
        if (keyboard != null && keyboard[Key.N].wasPressedThisFrame)
        {
            ...
            ShowVictory(TestGameEndInfoJson);
        }
    }
    ...
    {
        overlayRoot = new GameObject("GameEndVictoryOverlay", typeof(RectTransform), typeof(CanvasGroup), typeof(Image));
        ...
        BuildVictoryTitle(overlayRoot.transform);
    }

    private void BuildPanel(Transform parent)
    {
        panelRoot = CreateRect("VictoryPanelRoot", parent);
    ...
    {
        statsRowsRoot = CreateRect("VictoryStatRows", parent);
        ...
        var likeSprite = ResolveSprite(statPluginResourcePath, statPluginAssetPath, statLikeSpriteName);
        ...
        {
            var row = CreateRect($"VictoryStatRow_{i + 1}", statsRowsRoot);
    ...
    {
        var stamp = CreateImage("VictoryGradeStamp", parent, null);
    ...
    private void BuildVictoryTitle(Transform parent)
    {
        var title = CreateImage("VictoryTitleHat", parent, ResolveSprite(victoryTitleResourcePath, victoryTitleAssetPath, victoryTitleSpriteName));
    ...
    private IEnumerator PlaySingleStatRowSequence(int index)
    {
        ...
        if (statRowsData != null && index < statRowsData.Length && statRowsData[index].liked)
        {
    ...
    private IEnumerator PlayGradeStampIntro()
    {
        ...
        float power = likeCount >= 5 ? 1.28f : likeCount == 4 ? 1.12f : 1f;
        ...
        stampRect.localScale = Vector3.one * 3.35f * power;
        ...
        float elapsed = 0f;
        float windupSeconds = Mathf.Max(0.01f, stampIntroSeconds * 0.18f);
        while (elapsed < windupSeconds)
        {
            elapsed += Time.unscaledDeltaTime;
            float t = Mathf.Clamp01(elapsed / windupSeconds);
            stampRect.localScale = Vector3.one * Mathf.Lerp(3.35f * power, 3.65f * power, Mathf.SmoothStep(0f, 1f, t));
            stampRect.localRotation = Quaternion.Euler(0f, 0f, Mathf.Lerp(-12f, -16f, t));
            stampRect.anchoredPosition = Vector2.Lerp(startPos, startPos + new Vector2(0f, 10f), t);
            if (stampCanvasGroup != null)
            {
        ...
        elapsed = 0f;
        float slamSeconds = Mathf.Max(0.01f, stampIntroSeconds * 0.22f);
        while (elapsed < slamSeconds)
        {
            elapsed += Time.unscaledDeltaTime;
            float t = Mathf.Clamp01(elapsed / slamSeconds);
            float eased = t * t * t;
            stampRect.localScale = Vector3.one * Mathf.LerpUnclamped(3.65f * power, 1f, eased);
            stampRect.localRotation = Quaternion.Euler(0f, 0f, Mathf.LerpUnclamped(-16f, 2f, eased));
            stampRect.anchoredPosition = Vector2.LerpUnclamped(startPos + new Vector2(0f, 10f), restPos, eased);
            if (stampCanvasGroup != null)
            {
        ...
        stampRect.localRotation = Quaternion.Euler(0f, 0f, 2f);
    ...
    private IEnumerator PlayStampImpact(float power)
    {
        ...
        float elapsed = 0f;
        const float impactSeconds = 0.18f;
        while (elapsed < impactSeconds)
        {
            elapsed += Time.unscaledDeltaTime;
            float t = Mathf.Clamp01(elapsed / impactSeconds);
            float decay = 1f - t;
            float hit = Mathf.Sin(t * Mathf.PI * 8f) * decay;
            panelRoot.anchoredPosition = panelRestPos + new Vector2(hit * 8f * power, Mathf.Abs(hit) * -5f * power);
            if (stampRect != null)
            {
                stampRect.anchoredPosition = stampRestPos + new Vector2(hit * 2.5f * power, Mathf.Abs(hit) * -1.5f * power);
            }
            yield return null;
        }
    ...
    private void PrepareStatRows()
    {
        ...
        statRowsData = new VictoryStatRowData[count];
        var info = ParseGameEndInfo(string.IsNullOrWhiteSpace(gameEndInfoJson) ? TestGameEndInfoJson : gameEndInfoJson);
        var stats = info != null && info.stats != null ? info.stats : ParseGameEndInfo(TestGameEndInfoJson).stats;

        for (int i = 0; i < count; i++)
        {
            if (i == 0)
            {
                int value = Mathf.Max(0, stats.roundCount);
                statRowsData[i] = new VictoryStatRowData
                {
            ...
            if (i == 1 || i == 2)
            {
                var item = i == 1 ? stats.bestSellingItem : stats.mostPurchasedItem;
                int value = Mathf.Max(0, item != null ? item.quantity : 0);
                statRowsData[i] = new VictoryStatRowData
                {
                    displayValue = value.ToString(),
                    numericValue = value,
            ...
            if (i == 3)
            {
                int value = Mathf.Max(0, stats.totalSoldQuantity);
                statRowsData[i] = new VictoryStatRowData
                {
                    displayValue = value.ToString(),
                    numericValue = value,
    ...
    private static Sprite ResolveSprite(string resourcePath, string assetPath, string spriteName)
    {
        ...
        Debug.LogWarning($"[GameEndVictoryUI] UI sprite not found: {assetPath} ({spriteName})");
        return null;
    }
    ...
    private struct VictoryStatRowData
    {
